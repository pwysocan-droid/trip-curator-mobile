...
export default function ExperienceDetail() {
  ...
  useEffect(() => {
    ...
    fetchRegionData(region)
      .then((data) => {
        setItem(data.experiences.find((e: Experience) => e.id === id) || null)
        setLoading(false)
      })
  }, [id])

  if (loading) return <div className={styles.loading}>Loading…</div>
  if (!item) return <div className={styles.loading}>Experience not found.</div>
          ...
          <div className={styles.section}>
            ...
            <div className={styles.mapContainer}>
              <MapWidget
                ...
                markers={[{lat: item.lat, lng: item.lng, name: item.name, type: 'experience'}]}
                radiusKm={0}
              />
            </div>
          </div>
        ...
        <div className={styles.sidebar}>
          <div className={styles.bookingCard}>
            <div className={styles.bookingPrice}>
              ...
              <span className={styles.priceUnit}> / guest</span>
            </div>
