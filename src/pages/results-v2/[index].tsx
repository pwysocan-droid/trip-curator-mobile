...
export default function TripPageV2() {
  ...
  useEffect(() => {
    ...
    if (safeIndex >= trips.length) {
      router.replace({pathname: `/results-v2/0`, query: {n: nights}}, undefined, {shallow: true})
    }
    ...
    <div className="frame">
      ...
      {isFirstTrip ? (
        ...
        <section className={styles.compactInspiration}>
          {trip?.stay.images[0]?.url && (
            <img className={styles.compactThumb} src={trip.stay.images[0].url} alt={trip.stay.name} />
          )}
